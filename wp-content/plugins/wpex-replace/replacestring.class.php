...
class WPEXReplaceTextInTables extends WP_List_Table
{
	...
	function findURls()
	{
		...
		$result = array();
		$regex = "!((?:".implode('|', array_keys($replaceList)).")[^#?/\"]*)(\/wp-content)?!";
...
/*			
			...
			<div id="'.$pos.'" class="row">
				<input type="text" id="text_'.$pos.'" name="text_'.$pos.'" value="'.$url.'" readonly="readonly"';
			...
			$htmlContent .= ';" />
				<input type="text" id="newtext_'.$pos.'" name="newtext_'.$pos.'" value="" />
				<input type="button" class="button add" id="btntext_'.$pos.'" name="btntext_'.$pos.'" value="'.__('Exclude', 'wpex-replace').'" />
			</div>';
...
*/			
  ...
  {
    return '<a class="found-url" href="'.$item['found'].'" target="_blank">'.$item['found'].'</a><input type="hidden" name="org_url[]" value="'.$item['found'].'" />';
  }
	
  function column_replace($item)
  {
    return '<input type="text" name="replace[]" value="" /><input type="hidden" name="ids[]" value="'.$item['tables'].'" />';
  }
