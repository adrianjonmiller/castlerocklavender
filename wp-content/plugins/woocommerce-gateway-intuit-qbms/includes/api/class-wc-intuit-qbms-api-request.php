...
class WC_Intuit_QBMS_API_Request extends XMLWriter implements SV_WC_Payment_Gateway_API_Request {
	...
	public function customer_check_debit( $order ) {
		...
		$this->writeElement( 'InvoiceID', ltrim( $order->get_order_number(), _x( '#', 'hash before order number', WC_Intuit_QBMS::TEXT_DOMAIN ) ) );
		...
		if ( $order->customer_note ) {
			$this->writeElement( 'Comment', $order->customer_note );
		}
	...
	public function wallet_add( $order ) {
		...
		if ( 'credit_card' == $order->payment->type ) {
			...
			$this->writeElement( 'NameOnCard',           substr( $order->billing_first_name . ' ' . $order->billing_last_name, 0, 30 ) );
			$this->writeElement( 'CreditCardAddress',    substr( empty( $order->billing_address_2 ) ? $order->billing_address_1 : $order->billing_address_1 . ' ' . $order->billing_address_2, 0, 30 ) );
			$this->writeElement( 'CreditCardCity',       substr( $order->billing_city, 0, 50 ) );
			$this->writeElement( 'CreditCardState',      substr( $order->billing_state, 0, 20 ) );
			$this->writeElement( 'CreditCardPostalCode', substr( str_replace( '-', '', $order->billing_postcode ), 0, 9 ) );
	...
	private function credit_card_charge_auth_request( $request_type, $order ) {
		...
		if ( isset( $order->payment->token ) && $order->payment->token ) {
			...
			$this->writeElement( 'WalletEntryID', $order->payment->token );
			$this->writeElement( 'CustomerID',    $order->customer_id );  // TODO is this supposed to be different from UserID?
			...
			$this->writeElement( 'NameOnCard',           substr( isset( $order->intuit_qbms_test_condition ) ? 'configid=' . $order->intuit_qbms_test_condition : $order->billing_first_name . ' ' . $order->billing_last_name, 0, 30 ) );
			$this->writeElement( 'CreditCardAddress',    substr( empty( $order->billing_address_2 ) ? $order->billing_address_1 : $order->billing_address_1 . ' ' . $order->billing_address_2, 0, 30 ) );
			$this->writeElement( 'CreditCardCity',       substr( $order->billing_city, 0, 50 ) );
			$this->writeElement( 'CreditCardState',      substr( $order->billing_state, 0, 20 ) );
			$this->writeElement( 'CreditCardPostalCode', substr( str_replace( '-', '', $order->billing_postcode ), 0, 9 ) );
		...
		$this->writeElement( 'InvoiceID', substr( ltrim( $order->get_order_number(), _x( '#', 'hash before order number', WC_Intuit_QBMS::TEXT_DOMAIN ) ), 0, 100 ) );
		...
		if ( $order->customer_note ) {
			$this->writeElement( 'Comment', substr( $order->customer_note, 0, 4000 ) );
		}
...
} // end WC_Intuit_QBMS_API_Request class
