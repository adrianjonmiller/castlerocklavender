...
class WC_Intuit_QBMS_API implements SV_WC_Payment_Gateway_API {
	...
	private function perform_request( $request, $response_class_name ) {
		...
		$wp_http_args = array(
			...
			'sslverify'   => false,
			'blocking'    => true,
			'user-agent'  => "WooCommerce/" . SV_WC_Plugin_Compatibility::get_wc_version(),
			'headers'     => array(
		...
		if ( 200 != $response['response']['code'] ) {
			...
			do_action( 'wc_' . $this->id . '_api_request_performed', $request_data, $response_data );  // TODO: probably will need to vary this for cc/echeck gateways
