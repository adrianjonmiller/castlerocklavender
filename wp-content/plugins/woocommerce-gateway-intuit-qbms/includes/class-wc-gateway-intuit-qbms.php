...
class WC_Gateway_Intuit_QBMS extends SV_WC_Payment_Gateway_Direct {
	...
	protected function maybe_add_settings_descriptions() {
		...
		$application_login_description = $this->application_login ? '' :
			sprintf( __( 'You can create your own production Application Login and Application ID in the %sIntuit Partner Program%s, or use the default.  This is optional.', WC_Intuit_QBMS::TEXT_DOMAIN ),
				'<a href="https://developer.intuit.com">', '</a>' );

		$connection_ticket_description =
			sprintf( __( 'Generate a new production connection ticket for the configured Application ID by following %sthis link%s', WC_Intuit_QBMS::TEXT_DOMAIN ),
		...
		$test_application_login_description = $this->test_application_login ? '' :
			sprintf( __( 'You can create your own test Application Login and Application ID in the %sIntuit Partner Program%s, or use the default.  This is optional.', WC_Intuit_QBMS::TEXT_DOMAIN ),
					'<a href="https://developer.intuit.com">', '</a>' );

		$test_connection_ticket_description =
			sprintf( __( 'Generate a new test connection ticket for the configured Application ID by following %sthis link%s', WC_Intuit_QBMS::TEXT_DOMAIN ),
	...
	protected function get_method_form_fields() {
		...
		return array(
			...
			'application_login' => array(
				'title'    => __( 'Application Login', WC_Intuit_QBMS::TEXT_DOMAIN ),
				'type'     => 'text',
				'class'    => 'environment-field production-field',
				'desc_tip' => __( 'Your application login value, this will be something like appname.domainname.com', WC_Intuit_QBMS::TEXT_DOMAIN ),
				'placeholder' => self::PRODUCTION_APP_LOGIN_DEFAULT,
			),

			'application_id' => array(
				'title'    => __( 'Application ID', WC_Intuit_QBMS::TEXT_DOMAIN ),
				'type'     => 'text',
				'class'    => 'environment-field production-field',
				'desc_tip' => __( 'Your application ID value.', WC_Intuit_QBMS::TEXT_DOMAIN ),
				'placeholder' => self::PRODUCTION_APP_ID_DEFAULT,
			),

			'connection_ticket' => array(
				'title'    => __( 'Connection Ticket', WC_Intuit_QBMS::TEXT_DOMAIN ),
				'type'     => 'password',
				'class'    => 'environment-field production-field',
				'desc_tip' => __( 'Your unique connection ticket value.', WC_Intuit_QBMS::TEXT_DOMAIN ),
			),
	 ...
	 * @see WC_Gateway_Intuit_QBMS::order_meta_box_transaction_link()
	...
	public function get_transaction_url( $order ) {
		...
		return $merchant_center_url . '/portal/authsec/portal/qbms/Search+and+reporting/Search+for+advanced+transactions/SearchForAdvTransactions?action=1';
	 ...
	 * merchant account.  Intuit QBMS is a little more complicated than simply
	 * linking to transaction, instead we must post a search request to return
	 * the transaction we want to view
	 *
	...
	public function order_meta_box_transaction_link( $order ) {
		...
		// add the javascript code to submit the transaction search form when the "View Transaction" button is clicked
		SV_WC_Plugin_Compatibility::wc_enqueue_js( '$( "#js-wc-' . $this->get_plugin()->get_id_dasherized() . '-view-transaction" ).click( function() { $( "#js-wc-' . $this->get_plugin()->get_id_dasherized() . '-transaction-search" ).submit(); return false; } );' );

		// render the "View Transaction" button
		?>
		<li class="wide" style="text-align: center;">
			<a class="button tips" id="js-wc-<?php echo $this->get_plugin()->get_id_dasherized(); ?>-view-transaction" href="#" data-tip="<?php esc_attr_e( 'Search for this transaction in your merchant account', WC_Intuit_QBMS::TEXT_DOMAIN ); ?>" style="cursor: pointer !important;"><?php printf( __( 'View in %s', WC_Intuit_QBMS::TEXT_DOMAIN ), $this->get_method_title() ); ?></a>
		</li>
		<?php
	}
	...
	public function get_connection_ticket() {
		return preg_replace( '/[\x00-\x20\x7F-\xFF]/', '', $this->is_production_environment() ? $this->connection_ticket : $this->test_connection_ticket );
	}
