...
<fieldset>
	...
	if ( $tokens ) : ?>
		<p class="form-row form-row-wide">
			<a class="button" style="float:right;" href="<?php echo get_permalink( woocommerce_get_page_id( 'myaccount' ) ); ?>#wc-intuit-qbms-my-payment-methods"><?php echo wp_kses_post( apply_filters( 'wc_gateway_intuit_qbms_manage_my_payment_methods', __( "Manage My Cards", WC_Intuit_QBMS::TEXT_DOMAIN ) ) ); ?></a>
			<?php foreach( $tokens as $token ) : ?>
				<input type="radio" id="wc-intuit-qbms-payment-token-<?php echo esc_attr( $token->get_token() ); ?>" name="wc-intuit-qbms-payment-token" class="js-wc-intuit-qbms-payment-token js-wc-payment-gateway-payment-token" style="width:auto;" value="<?php echo esc_attr( $token->get_token() ); ?>" <?php checked( $token->is_default() ); ?>/>
				<label style="display:inline;" for="wc-intuit-qbms-payment-token-<?php echo esc_attr( $token->get_token() ); ?>"><?php printf( __( '%s ending in %s (expires %s)', WC_Intuit_QBMS::TEXT_DOMAIN ), $token->get_image_url() ? '<img src="' . esc_url( $token->get_image_url() ) . '" />' : esc_html( $token->get_type_full() ), esc_html( $token->get_last_four() ), esc_html( $token->get_exp_month() . '/' . $token->get_exp_year() ) ); ?></label><br />
			<?php endforeach; ?>
			<input type="radio" id="wc-intuit-qbms-use-new-payment-method" name="wc-intuit-qbms-payment-token" class="js-wc-intuit-qbms-payment-token" style="width:auto;" value="" <?php checked( $default_new_card ); ?> /> <label style="display:inline;" for="wc-intuit-qbms-use-new-payment-method"><?php echo __( 'Use a new credit card', WC_Intuit_QBMS::TEXT_DOMAIN ); ?></label>
		</p>
		<div class="clear"></div>
	<?php endif; ?>
	<div class="wc-intuit-qbms-new-payment-method-form js-wc-intuit-qbms-new-payment-method-form" <?php echo ( $tokens ? 'style="display:none;"' : '' ); ?>>
		<p class="form-row form-row-first">
			<label for="wc-intuit-qbms-account-number"><?php _e( "Credit Card Number", WC_Intuit_QBMS::TEXT_DOMAIN ); ?> <span class="required">*</span></label>
		...
		<p class="form-row form-row-last">
			<label for="wc-intuit-qbms-exp-month"><?php _e( "Expiration Date", WC_Intuit_QBMS::TEXT_DOMAIN ); ?> <span class="required">*</span></label>
			<select name="wc-intuit-qbms-exp-month" id="wc-intuit-qbms-exp-month" class="js-wc-payment-gateway-card-exp-month" style="width:auto;">
				<option value=""><?php _e( 'Month', WC_Intuit_QBMS::TEXT_DOMAIN ) ?></option>
			...
			<select name="wc-intuit-qbms-exp-year" id="wc-intuit-qbms-exp-year" class="js-wc-payment-gateway-card-exp-year" style="width:auto;">
				<option value=""><?php _e( 'Year', WC_Intuit_QBMS::TEXT_DOMAIN ) ?></option>
		...
		<?php if ( $enable_csc ) : ?>
			<p class="form-row form-row-wide">
				<label for="wc-intuit-qbms-csc"><?php _e( "Card Security Code", WC_Intuit_QBMS::TEXT_DOMAIN ) ?> <span class="required">*</span></label>
		...
		if ( $tokenization_allowed || $tokenization_forced ) :
			if ( $tokenization_forced ) :
				...
				<p class="form-row">
					...
					<label for="wc-intuit-qbms-tokenize-payment-method" style="display:inline;"><?php echo wp_kses_post( apply_filters( 'wc_gateway_intuit_qbms_tokenize_payment_method_text', __( "Securely Save Card to Account", WC_Intuit_QBMS::TEXT_DOMAIN ) ) ); ?></label>
				</p>
				<div class="clear"></div>
				<?php
			endif;
